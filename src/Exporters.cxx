	...
	if (fp) {
		...
		for (int istyle = 0; istyle <= STYLE_DEFAULT; istyle++) {
			...
			if ((valdef && *valdef) || (val && *val)) {
				...
				if(fore.length()) {
					...
					Platform::DebugPrintf("rgb => %s", fore.c_str());
					
					// red component...
					buffer[0] = fore[1]; buffer[1] = fore[2]; buffer[2] = '\0';
					sscanf(buffer,"%x",&red);

					// green component...
					buffer[0] = fore[3]; buffer[1] = fore[4]; buffer[2] = '\0';
					sscanf(buffer,"%x",&green);
					
					// blue component...
					buffer[0] = fore[5]; buffer[1] = fore[6]; buffer[2] = '\0';
					sscanf(buffer,"%x",&blue);

					// debugging...
					Platform::DebugPrintf("r, g, b => %d,%d,%d\n",red,green,blue);
		...
		for (int i = 0; i < lengthDoc; i++) {
			...
			if (ch == '\t') {
				...
				// update the number of lines counter
				numLines++;
			} else if (ch == '\n') {
				if (!prevCR) {
					stream += ") Tj\n";
		...
		// couldnt open the file for saving,
